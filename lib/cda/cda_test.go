...
func TestGetLatestCompleteRecord_SkipsLaggedRow(t *testing.T) {
	h0 := time.Date(2026, 9, 17, 12, 0, 0, 0, time.UTC).UnixMilli()
	h1 := time.Date(2026, 9, 17, 13, 0, 0, 0, time.UTC).UnixMilli()
	...
	srv := httptest.NewServer(serveSeries(t, map[string][][2]float64{
		...
		tsTurbine:    {{float64(h0), 100.0}, {float64(h1), 110.0}},
		tsSpillway:   {{float64(h0), 0.0}, {float64(h1), 0.0}},
		// Total lags: the newest timestamp (h1) is incomplete.
		tsTotal: {{float64(h0), 100.0}},
	}))
	defer srv.Close()

	c := newTestClient(srv)
	rec, err := c.GetLatestCompleteRecord(context.Background(),
		time.Date(2026, 9, 17, 0, 0, 0, 0, time.UTC),
		time.Date(2026, 9, 17, 23, 0, 0, 0, time.UTC))
	if err != nil {
		t.Fatalf("GetLatestCompleteRecord: %v", err)
	}
	// Must return h0 (complete), not the newer-but-incomplete h1.
	if got := rec.MeasuredAt.AsTime().UTC().Hour(); got != 12 {
		t.Errorf("returned hour %d, want 12 (the last complete row, not the lagged h1)", got)
	}
	if rec.Level != 912.30 || rec.TotalReleaseRate != 100 {
		t.Errorf("record = level %v total %v, want 912.30 / 100", rec.Level, rec.TotalReleaseRate)
	}
}

func TestGetLatestCompleteRecord_NoneComplete(t *testing.T) {
	h0 := time.Date(2026, 9, 17, 12, 0, 0, 0, time.UTC).UnixMilli()
	srv := httptest.NewServer(serveSeries(t, map[string][][2]float64{
		tsElevation: {{float64(h0), 912.30}},
		// generation/turbine/spillway/total all missing → no complete row.
	}))
	defer srv.Close()

	c := newTestClient(srv)
	_, err := c.GetLatestCompleteRecord(context.Background(),
		time.Date(2026, 9, 17, 0, 0, 0, 0, time.UTC),
		time.Date(2026, 9, 17, 23, 0, 0, 0, time.UTC))
	if err == nil {
		t.Fatal("expected an error when no row is complete, got nil")
	}
}
